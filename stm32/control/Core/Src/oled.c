...
// OLED器件地址：SSD1306 常见 0x78（7位 0x3C），CH1116 常见 0x7A（7位 0x3D）
// 上电自动探测（见 OLED_ProbeAddress），兼容两种屏
#define OLED_ADDRESS_DEFAULT 0x78
static uint8_t g_oled_addr = OLED_ADDRESS_DEFAULT;
...
/**
 * @brief 探测 OLED 实际 I2C 地址（SSD1306=0x78 / CH1116=0x7A）
 * @note 用 HAL_I2C_IsDeviceReady 发地址试探，哪个有 ACK 用哪个；
 *       都无响应返回默认 0x78（此时需检查接线/供电/上拉）
 */
uint8_t OLED_ProbeAddress(void)
{
  if (HAL_I2C_IsDeviceReady(&hi2c1, 0x78, 3, 50) == HAL_OK) return 0x78;
  if (HAL_I2C_IsDeviceReady(&hi2c1, 0x7A, 3, 50) == HAL_OK) return 0x7A;
  return OLED_ADDRESS_DEFAULT;
}
...
void OLED_Send(uint8_t *data, uint8_t len) {
  HAL_I2C_Master_Transmit(&hi2c1, g_oled_addr, data, len, HAL_MAX_DELAY);
}
 ...
 */
void OLED_Init() {
  g_oled_addr = OLED_ProbeAddress();   /* 自动探测 0x78 / 0x7A */
  ...
  /* 电荷泵：SSD1306 用 0x8D/0x14，CH1116 用 0xAD/0x8B，两个都发保证兼容 */
  OLED_SendCmd(0x8D); /*SSD1306 电荷泵使能*/
  OLED_SendCmd(0x14);
  OLED_SendCmd(0xAD); /*设置启动电荷泵 set charge pump enable (CH1116)*/
...
/* 仅刷新指定页（page 0~7）：避免整帧 8 页刷新阻塞主循环（PID 10ms 周期）。
 * 单页 131 字节 @400kHz ≈ 3ms，可放在闭环循环内安全使用 */
