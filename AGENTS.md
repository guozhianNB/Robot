...
- **后端专用 rosbridge（:9091，`nav_screen.sh lat2`）没起 = 车控 / 位姿 / 病房自动切换一起断**：`.env` 的 `ROSBRIDGE_URL` 指向 9091（9090 留给 Foxglove 的 `lat`）。**`nav_screen.sh kill all` 会连 lat2 一起停且不自动恢复**，而后端启动时不做网络探测，所以症状要等到调用时才现形：`ok:false`「车体未就绪: [Errno 111] Connection refused」——**长得像"车坏了"，其实是 rosbridge 没起**。排查先 `ss -lnt | grep 9091`，别再查车。
