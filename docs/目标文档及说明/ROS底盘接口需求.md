...
| 当前位置/区域 | `robot/current_place` | `place`（语义区域名） | 知道"在 2 号病房 / 13 号床前"（见四·1） |
...
### 四·1 语义区域标注（坐标 → 区域，回答"我在哪"）

第四节是"地点→坐标"（去某个地方），本节是**反向**：给定坐标判断"小车在哪个区域"。用于吃药提醒（定位到床前）、主动问候、对话上下文。

- 在地图上把房间/床位画成**坐标区域框**（矩形/多边形），命名：

  ```json
  {
    "2号病房":       { "x_min": 1.0, "x_max": 4.0, "y_min": 1.0, "y_max": 4.0 },
    "2号病房13号床": { "x_min": 1.0, "x_max": 1.5, "y_min": 1.0, "y_max": 2.0 },
    "客厅":         { "x_min": 6.0, "x_max": 9.0, "y_min": 0.0, "y_max": 3.0 }
  }
  ```

- 大模型端接口 `get_location()` → 由定位坐标匹配区域框，返回语义名（如"2号病房13号床前"）
- 同一坐标可落在多个层级（病房 / 床），返回最细粒度 + 上级
- 由方向二建图后标注生成，大模型端消费
...
6. 语义区域框（病房/床位）由谁标注、放哪个节点加载？坐标→区域匹配放 ROS 侧还是大模型端？
