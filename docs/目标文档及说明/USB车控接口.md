...
**电机安装方向（硬件）**：为便于安装摆放，对角两台电机**反放**——**左前(LF, way1)、右后(RR, way4) 电机转向与车架相反**。已在 `motor_driver.c` 的 `md_dir_pin` 表交换这两台的 IN1/IN2 引脚修正；若换电机/重新接线后方向再次相反，改该表即可（编码器方向由上电自动标定，无需另改）。
