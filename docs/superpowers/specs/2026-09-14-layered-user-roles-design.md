...
> 日期：2026-09-14（含当晚用户追加需求：集体层=病房用户、口令可改可关、MCP 暂缓）｜ **2026-09-14 二次复审修订**（结论与逐条处置见 **§14**：修正与代码现状的 7 处冲突——病房区域几何载体、位置源配置项、`locator.py` 已存在、`profiles` 关联列、地点真相载体、`db` 既有函数签名、测试基线）｜ **状态：已实现 P0（2026-09-14）**（落地事实见 **§15**）｜ 范围：后端会话与权限层 + 提示词分层 + 前端层级切换 + 集体层（病房用户）
...
## 15. 实现落地说明（2026-09-14）

> 本节性质：正文 §1～§13 是**设计意图**，§14 是**复审结论**，本节是**实际落地事实**。凡与正文冲突，以本节（与计划文末「实现台账与偏差」）为准。实现计划：`docs/superpowers/plans/2026-09-14-layered-user-roles.md`（v2，15 任务，已执行完）。

### 15.1 代码落在哪些提交

P0 全部 15 个任务落在 **`b83c594`（任务 1 `zonegeo.py`）→ `5c3e1db`（任务 14 admin 登录门与两个页签）**，
基线为 `6295df3`（2026-09-14 20:23）。区间内混有**并行会话**的 vision / mapeditor 提交
（`bbfcb8b`/`ff82042`/`29fa314`/`a69aba7`/`3516839`），**不属于本批**，清点本批提交时须剔除。

### 15.2 与规格的已知偏差

| # | 偏差 | 原因 |
|---|---|---|
| 1 | `GET /api/session/user` 走 `asyncio.to_thread` | `autoswitch_state()` 会碰位姿（`locator.get_pose()` 最多 drain 0.8s）并反查"车在跑哪张图"（`MAPS_IO=ssh` 下含远端 stat/整图拉取），占住事件循环会拖慢整个后端 |
| 2 | 解锁（`POST /api/session/user {locked:false}`）= 回**当前病房**的集体层 | 即 §4.5 语义；**前端传的 uid 被忽略**（不能借解锁把主体设成任意 uid） |
| 3 | 改口令：**只要已设过口令就必须验旧口令**，与口令门开关无关 | 堵"关门 → 改口令 → 开门"永久占住口令的链（首启态尚未设过口令才允许免旧口令直接设） |
| 4 | `POST /api/session/user` 额外拒绝 `uid="admin"`；`X-Surface` 非法值返回 400 | 前者：`set_subject()` 拒绝提权时返回的是**当前主体**（与成功同形），前端会误判"切到管理员成功"；后者：静默回落 kiosk 会把管理台的口令登录写进车前屏槽 |
| 5 | `remote`-MCP 工具纳入角色白名单**交集**（白名单是天花板），MCP 服务器未声明 `roles` = 仅 admin | §3.3 能力矩阵优先于 MCP 自身声明；未声明即 fail-closed |
| 6 | 集体层白名单 = `["robot_status","robot_stop"]`，**不是空列表** | R3：急停/呼救必须永远可用；空列表等于连急停都做不了 |

### 15.3 待办

- **P1（暂缓，D16）**：car MCP 新增 `robot_goto`（§7.2）+ `destinations` 与地点白名单解析 + 风险分级
  + 二次确认状态机 + admin「地点白名单」页签（§6.3 / §7）。用户 2026-09-14：「先不管 mcp，先完成用户系统」。
- **用户侧前端复验**：本机（沙箱）`frontend` 下跑 `pnpm test` / `pnpm -r build` 会因
  `esbuild@0.21.3` 的 `spawn EPERM` 直接失败（环境限制，非代码问题），故三个前端包的 vitest 与
  `vite build` **均未在本机复验**——需用户执行 `cd frontend && pnpm test && pnpm -r build` 后确认。
- **真机联调**：rosbridge 真位姿 + 真地图指纹下的病房自动切换（本机只用注入假位姿验过），
  以及声纹→角色→提示词分层这条链的麦克风实测，随 P1/导航线一并做。
