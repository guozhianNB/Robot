...
> 日期：2026-08-27 ｜ 状态：待用户审查 ｜ 范围：前端重构 + `/api/alarm` + 会话状态（`active_uid`/锁定）
> 关联：`docs/superpowers/specs/2026-08-18-ai-chat-frontend-design.md`（旧单文件前端）、
> `docs/superpowers/specs/2026-08-22-voice-pipeline-design.md`（语音链路，会话层小改动）、
...
2. **无屏优先**：屏在但老人不用屏、语音是主交互通道；屏幕是"跟随显示器"，但**保留完整触摸交互能力**（对话操作/切换用户/设置）
3. **工程化**：消除单文件带来的维护痛点（尤其 SSE 事件协议两端强耦合）

**关键架构原则（已确认）：交互闭环在后端，前端既是订阅者也是操作者。** 语音链路（唤醒→ASR→LLM→TTS）已在后端 `voice/` 模块闭环，不依赖任何前端——屏幕崩溃、屏没装、老人不用屏，语音链路照常工作。同时前端通过 REST 向后端发送操作（发消息、切用户、改设置），通过 SSE 订阅状态变化。两条通道职责分明：

| 通道 | 方向 | 用途 |
|---|---|---|
| REST API（`/api/chat`、`/api/session/user`…） | 前端 → 后端 | 用户操作（写） |
| SSE 事件总线（`/api/events`） | 后端 → 前端 | 状态变化广播（读/订阅） |
...
| D4 | 语音交互 | 后端闭环不变；屏幕可随时缺席，语音链路不依赖前端 |
| D5 | kiosk 触摸深度 | **展示 + 完整交互**：对话操作、手动切换用户、进入设置界面、提醒确认、SOS 紧急呼叫 |
| D6 | 紧急呼叫 | 本期只做 `POST /api/alarm` 端点 + 审计 + 事件广播；微信推送留给模块 11 |
| D7 | 旧前端处置 | 保留在 `UI(old)/` 不删，作为参考实现；admin 功能齐平后仍保留 |
| D8 | 文档范围 | 前端重构 + `/api/alarm` + 会话状态（`active_uid`/锁定）；语音链路仅 worker 会话层小改动 |
| D9 | 部署形态 | 构建产物由 FastAPI StaticFiles 托管（复用 8000 端口），板卡 Chromium kiosk + systemd |
| D10 | 迁移策略 | 双轨运行，按功能逐个重写（不搬代码），接口契约不变 |
| D11 | 身份冲突策略 | **手动选择 + 锁定模式**：屏幕可手动选用户并锁定；锁定时声纹识别只提示不切换，解锁后恢复声纹自动判定 |
...
│  └─ + POST /api/session/user（本期新增：active_uid + 锁定，见 §8）                │
│                                                                                   │
└──────────────┬──────────────────────────────────────┬─────────────────────────────┘
               │ 主交互通道：语音（不经过屏幕）           │ REST 写 + SSE 订阅（屏幕可操作）
               ▼                                        ▼
        麦克风/扬声器                             ┌──────────────────┐
        唤醒→对话→播报，全闭环                     │ kiosk 车载屏端    │
        屏幕挂了/关了完全不影响                    │ 状态条/对话区/    │
                                                 │ 切换用户/设置/    │
                                                 │ 提醒确认/SOS      │
                                                 └──────────────────┘
                                                 ┌──────────────────┐
...
1. 语音交互回路（唤醒→识别→对话→播报）在后端 `voice/` 内部闭环，不经过屏幕 → 屏幕崩溃不影响语音
2. 前端操作走 REST（发消息/切用户/改设置），状态变化走 SSE 广播（提醒触发/语音状态/用户切换），两条通道职责分明——**前端既是订阅者也是操作者**
3. 屏幕的独特价值 = 低置信度身份确认（声纹置信度低时语音反问 + 屏幕弹候选身份卡片，护工/家属一键确认）——`identity.py` 融合层"宁问勿猜"的视觉出口
4. 多端共享 `shared/` 的 API client 与事件解析，SSE 协议定义只写一份
...
export type BusEvent =
  ...
  | { type: "user_changed"; payload: { uid: string; locked: boolean; source: "manual" | "voiceprint" } }
...
**定位**：语音会话的实时可视化 + 完整触摸交互（对话/切用户/设置/提醒确认/SOS）。单页沉浸式 + 少数弹层/子页。

```
┌─────────────────────────────────────────┐
│  状态条：◉ 待机 | 正在听… | 播报中…      │  ← 映射 /api/voice/status（三色）
│          👤 张爷爷 🔒 | 李奶奶           │  ← active_uid + 锁定标记（可点切换）
├─────────────────────────────────────────┤
│                                         │
│   对话区（自动滚动，大字大行距）          │
│   👴 张爷爷：今天吃药了吗？（识别文本）    │
│   🤖 机器人：吃了，刚吃完降糖药……（回复） │
│   [输入框 + 发送]                        │  ← 手动对话（触摸键盘/快捷短语）
│                                         │
├─────────────────────────────────────────┤
│  底部常驻：提醒卡片轮播 + 时间/电量       │
│  [提醒确认] [设置] [SOS 紧急呼叫]        │  ← 触摸操作
└─────────────────────────────────────────┘
```

- **状态条**：直接映射后端语音状态机（IDLE/LISTENING/SPEAKING），三状态三色；右侧显示 `active_uid` + 锁定标记（🔒）
- **对话区**：SSE 订阅 `chat_new`，识别文本与回复实时上屏；底部输入框可手动发消息（`POST /api/chat`，touch keyboard / 快捷短语）——护工/家属既能看到也能操作
- **切换用户**：点击状态条用户 → 弹出 `/api/profiles` 列表 → 选人 → `POST /api/session/user {uid, locked:true}`（锁定模式，D11）→ `user_changed` 广播全端同步
- **设置界面**：简化版（语音开关、音量、亮度、唤醒词显示），与 admin 设置页共享后端但只暴露老人相关项
- **提醒确认**：提醒卡片触发时显示 `[确认]` → `POST /api/reminders/confirm`
...
## 8. 会话状态（active_uid + 锁定模式，D11）

**问题**：手动切换用户（屏幕）与声纹识别（语音链路）是两个身份来源，若各管各的会出现"屏幕切到张爷爷、声纹识别出李奶奶"的分裂。解决方案：后端统一会话状态。

### 8.1 现状（已实现）

`LLM/voice/worker.py` 已有 `self.current_uid`（语音线程内存态）+ `identity.py::effective_uid()`（"声纹优先、低置信沿用当前 uid"）。语音链路内部已维护"当前用户"，但前端无法读写它。

### 8.2 改动（约 20 行，voice 会话层小改）

```
worker.py:
  + self.locked_uid = None            # 手动锁定用户（None=未锁定）
  effective_uid() 逻辑改为：
    if locked_uid: return locked_uid  # 锁定时声纹只提示（audit）不切换
    return 现有逻辑                   # 未锁定：声纹优先

voice_api.py:
  + set_session_uid(uid, locked)      # 写 worker.locked_uid + 读 current_uid
  + get_session_uid()                 # 返回 {uid, locked, source}

server.py:
  + POST /api/session/user {uid, locked}  → set_session_uid + bus.publish("user_changed", ...)
  + GET  /api/session/user                → get_session_uid()
```
...
| 状态 | 语音对话归属 | 声纹识别到其他人 |
|---|---|---|
| 未锁定 | 声纹判定（现状不变） | 自动切换 uid |
| 锁定（🔒） | 固定为手动选择 uid | 只记 audit `voice_spk` 提示，**不切换**；解锁后恢复自动 |

**广播**：任何来源（手动/声纹）导致 `active_uid` 变化 → `bus.publish("user_changed", {uid, locked, source})` → kiosk 状态条、admin 全端同步。
...
| `server.py` 新增 `GET/POST /api/session/user` | 会话状态读写（见 §8.2） |
| `voice/worker.py` | 新增 `locked_uid` 字段 + `effective_uid` 锁定分支（§8.2） |
| `voice_api.py` | 新增 `set_session_uid` / `get_session_uid`（§8.2） |
| `bus.py` 确认 `alarm` 事件类型 | 已存在（reminder.py 在用），无需改动 |
| 其余后端 | 一律不动（对话编排、数据库、提醒调度均保持现状） |
...
- **会话状态**：`POST /api/session/user` 锁定/解锁行为矩阵（§8.2 表）、`user_changed` 广播、声纹识别到锁定外用户不切换
- **kiosk 端**：状态条三态渲染、SSE 断线重连、切换用户流程、SOS 按钮触发 `/api/alarm`、身份确认弹窗
...
| M4 | kiosk 端：状态条/对话区/提醒轮播/切换用户/设置/SOS/身份确认 | 语音状态实时上屏，切换用户全端同步，SOS 全链路通 |
| M5 | 后端：`POST /api/alarm` + `/api/session/user` + worker 锁定模式 + 集成 + 稳健性回归 | 审计/广播/锁定矩阵/降级验证通过 |
...
| kiosk 端 Chromium 性能（RDK X5） | 页面轻量（纯展示+少量交互），无重渲染；若不足再评估降级为 WebView 方案 |
| 锁定模式被误触（老人误锁用户） | 切换用户/设置入口需长按或护工模式解锁，SOS 始终一键直达 |
