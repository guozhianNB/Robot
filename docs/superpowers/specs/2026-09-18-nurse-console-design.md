...
packages/nurse/
  ...
  tsconfig.json
  src/main.ts
  src/App.vue                      壳：顶栏 + 筛选 + 列表 + 断线红条（D11：无登录门，打开即用）
...
3. 去重合并：同 `(source,type,uid,正文)` 60s 内两次 → 1 行、`count=2`、`body` 保持首次原文（**2026-09-18 修订**：正文进合并键，见 §4.2 注）
...
| **多台 PC 共享一个进程级 admin 槽**（管理台侧的直接后果） | 对 `/admin` 成立：第一台 PC 登录后，第二台 PC 打开 `/admin/` **不会**看到登录门——会话是后端进程级的，不是每浏览器一份。局域网内多屏共用一个管理员身份；要按人隔离需独立护士角色（另立规格）。护士台自 D11 起无登录概念，不受此影响 |
| **角标数可能大于可见卡片数** | 角标是**未处理总数**（全表 COUNT），列表默认上限 50 且面板**不做加载更多**（`before_id` 后端已留、面板不用）→ 未处理 >50 时"未处理 60"但只看到 50 张卡。符合 YAGNI，记此说明 |
| **持续复报长期合并为一行** | 去重窗口锚在 `last_at`：同一 `(source,type,uid,正文)` 每 <60s 复报会一直被合并、`count` 无限增长（一个持续故障 = 一行 + 次数），**ack 之后自然新开一行**。经裁定符合 D5 防刷屏本意 |
...
- admin 会话活动续期（原为 I5 的候选修法；D11 免登录后已无必要，见 §9 已解决行）
